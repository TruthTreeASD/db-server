...
public class LookUpControllerTests {
    ...
    public void findRecordsAtSameLevelTest(){
        ...
        assertEquals(locationInfoDTOS.get(0).toString(), ((List<LocationInfoDTO>)res.getData()).get(0).toString());
    }

    @Test
    public void findRecordsForSameParentTest(){
        List<Integer> attributeIds = new ArrayList<>();
        attributeIds.add(1);
        attributeIds.add(2);
        attributeIds.add(3);
        ...
        Mockito.when(locationRepository.findParentId(locationId)).thenReturn(100);
        Mockito.when(locationRepository.findTypeCode(locationId)).thenReturn(0);
        ...
        Mockito.when(locationRepository.findAllLocationsWithGivenParent(100, 0)).thenReturn(states);
        ...
        ResponseMessage res = lookUpService.findRecordsForSameParent(locationId, year, attributeIds);
        assertEquals(locationInfoDTOS.get(0).toString(), ((List<LocationInfoDTO>)res.getData()).get(0).toString());
